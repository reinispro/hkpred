      ...
      const fetchUserSettings = useCallback(async (userId) => {
        if (userId) {
          ...
          if (profileError && profileError.code !== 'PGRST116') {
            ...
            toast({ variant: 'destructive', title: 'Error fetching user profile', description: profileError.message });
            return null;
          }
          return profile;
        }
      ...
      const fetchAppSettings = useCallback(async () => {
         const { data: settingsData, error: settingsError } = await supabase
          ...
          if (settingsError) {
            ...
            toast({ variant: 'destructive', title: 'Error fetching app settings', description: settingsError.message });
          } else {
            const settingsMap = settingsData.reduce((acc, setting) => {
              acc[setting.setting_name] = { id: setting.id, is_enabled: setting.is_enabled, value_json: setting.value_json };
              return acc;
            }, {});
        ...
        return (
          <div className="min-h-screen w-full bg-gradient-to-br from-cyan-300 via-blue-500 to-indigo-600 flex flex-col items-center justify-center text-white p-4">
            <div className="text-center glass-card p-8 rounded-xl shadow-2xl max-w-md">
              ...
              <p className="text-lg mb-6">Thank you for registering! Your account is currently waiting for an administrator to approve it. Please check back later.</p>
